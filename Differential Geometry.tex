...
\begin{proof}
    ...
    \[
        \norm{\nabla^k f(x) }= \norm{\sum_{n = 1}^{\infty} \frac{(r_n)^{n - k}}{2^nM_n} \nabla^k H_n(x)} \leq  \sum_{n = k}^{\infty} \frac{(r_n)^{n - k}}{2^nM_n} \norm{\nabla^k H_n(x)} \leq \sum_{n = k}^{\infty} \frac{(r_n)^{n - k}}{2^nM_n}M_n = \sum_{n = k}^{\infty}\frac{(r_n)^{n - k}}{2^n},
    \]
    which is a convergent series, by the criteria of series, $\nabla^k f(x)$ is well-defined for all $k$, and is continuous. Hence $f$ is smooth. 
...
\subsection{Derivatives in Multivariable Calculus}
\begin{definition}
...
\end{proof}
\begin{definition}
    If $f: E \to \bb{R}$ is a real-valued function, and we define the gradient $\nabla f(x_0)$ of $f$ at $x_0$ to be the $n$-dimensional row vector 
    \[
        \nabla f(x_0):= \left(\de{f}{x_1}(x_0), \de{f}{x_2}(x_0), \dots, \de{f}{x_n}(x_0)\right)
    \]
\end{definition}
\subsection{Tangent Vectors of Euclidean Space}
...
\end{definition}
\begin{proposition}
    Let $a \in \bb{R}^n$, for each geometric tangent vector $v_a \in \bb{R}$, the map $\nabla \cdot v_a: C^{\infty}(\bb{R}^n)\to \bb{R}$ is a derivation at $a$. Moreover, the operation $v_a \mapsto \nabla(a) \cdot v_a$ is an isomorphism from $\bb{R}^n_a$ onto $T_a\bb{R}^n$.
\end{proposition}
\begin{proof}
    The fact $\nabla \cdot v_a$ is a consequence of the linearity and Lebniz implies that it is a derivation. It suffices to prove that $\mc{L}: v_a \mapsto \nabla \cdot v_a$ is an isomorphism. 

    For injectivity, suppose $\mc{L}(v_a) = \mc{L}(w_a)$ for some geometric tangent vector $v_a, w_a$, it folows that 
    \begin{align*}
        0 &= \mc{L}(v_a - w_a)(x^j) = \nabla \cdot (v_a - w_a)(x^j) =  \left(\frac{\partial}{\partial x^1}(x^j), \dots, \frac{\partial}{\partial x^n}(x^j)\right)\left(v_a - w_a\right)\\
        &= \frac{\pa}{\pa x_i}(x^j)(v^i -w^i) = \delta_{ij}(v^i - w^i) = v^j - w^j,
    \end{align*}
    for all $j = 1, \dots, n$. Thus $\mc{L}$ is injective. To prove surjectivity, let $w \in T_a\bb{R}^n$ be abitrary and let $v$ the the tangent vector of $\bb{R}^n_a$ such that $w(x^i)= v^i$, it suffices to prove that $w = \nabla\cdot v_a$. By the Taylor's theorem, one can write 
    \[
    ...
    \[
        w(f(x)) = w(f(a)) + \frac{\pa f}{\pa x^i}(a)(w(x^i) - a^i) = \frac{\pa f}{\pa x^i}(a)v^i = \nabla \cdot v_a (a)
    \]
