...
\begin{definition}
    A rectangle $R$ in $\bb{R}^n$ is a closed set given by 
    \[
        R = [a_1,b_1]\times \dots \times [a_n\times b_n]
    \]
\end{definition}
\begin{definition}The volume of $R$, denoted by $\vol(R)$, is defined to be 
    \[
...
\end{definition}
\begin{lemma}
    If $R = \displaystyle\bigsqcup_{k = 1}^nR_k$, then 
    \[
        \vol(R) = \sum_{k = 1}^n \vol(R_k)
    \]
\end{lemma}
...
\end{proof}
\begin{definition}
    Let $(X,\mc{M},\mu)$ be a measure space, a set $E \in \mc{M}$ such that $\mu(E) = 0$ is called a \textit{null set}. If a statement about points $x \in X$ is true except for $x$ in some null set, we say that it is true \textit{almost everywhere}, or for \textit{almost every} $x$.
\end{definition}
\begin{definition}
