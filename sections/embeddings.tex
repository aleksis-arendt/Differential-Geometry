...
\end{proposition}
\begin{proposition}
    Suppose $M^m$ and $N^n$ are smooth manifolds without boundary.
    \begin{enumerate}
        \item $F$ is a local diffeomorphism if and only if it is both a smooth immersion and a smooth submersion.
        \item If $m = n$, and $F$ is either a smooth immersion or a smooth submersion, then it is a local diffeomorphism.
    \end{enumerate}
\end{proposition}
...
\end{theorem}
\begin{theorem}[\textbf{Characteristic Property Theorem}]
...
\end{proposition}
\begin{lemma}\label{thm:torus-covering}
    Let $\ep^n: \bb{R}^n \to \bb{T}^n$ be the map satisfying 
    \[
        \ep^n(x^j) = \left(e^{2\pi i x^j}\right).
    \]
    We claim that $\ep^n$ is a smooth covering map.
\end{lemma}
\begin{proof}
    Observe that $\ep^n$ is smooth and surjective, it suffices to prove that $\ep^n$ is local diffeomorphism. For any $x \in \bb{R}^n$, consider the open cube 
    \[
        U_x = x + \left(-\frac{1}{2},\frac{1}{2}\right)^n
    \]
    Consider the restriction map $\ep^n|_{U_x} : U_x \to \ep^n(U_x)$. If $a,b \in U_x$ and $\ep^n(a) = \ep^n(b)$, it follows that $a - b \in \bb{Z}^n$ since $|a^i - b^i| < 1$ for all $i = 1,\dots, n$. Thus $\ep^n|_{U_x}$ is injective and has smooth inverse, thus it is diffeomorphism, following that $\ep^n$ is local diffeomorphism. Now we decay the preimage of $V = \ep^n(U)$ into union of disjoint connected subsets, which is
    \[
        \left(\ep^n\right)^{-1}(V) = \bigsqcup_{k \in \bb{Z}^n}(U_x + k)
    \]
    And since each map $:\ep^n: U_x + k \to \bb{T}^n$ is already a diffeomorphism, hence $\ep^n$ is indeed a smooth covering map.
\end{proof}
\section{Problems}
\begin{problem}
    Use the inclusion map $\bb{H}^n \hookrightarrow \bb{R}^n$ to show that the Inverse Function Theorem does not extend to the case which $M$ is a manifold with boundary.
\end{problem}
\begin{proof}
    Suppose the theorem holds for manifolds with boundary. Let $p = (x^1,\dots,x^{n - 1},0) \in \partial \bb{H}^n$, then there exists open neighborhoods $U_0$ and $V_0$ centered at $p$ and $F(p)$, respectively such that $F|_{U_0}:U_0 \to V_0$ is a diffeomorphism. This leads to contradiction since $U_0$ and $V_0$ have dimension $n$ and $n - 1$, respectively.
\end{proof}
\begin{problem}
...
\end{problem}
\begin{proof}
    Since $\pi$ is already surjective, it suffices to prove that it is smooth submersion. For each $i = 1,\dots, n +1$, let $U_i \subset N$ be the set of equivalent classes satisfying 
    \[
        U_i = \{[z^1,\dots,z^{n + 1}] \mid (z^1,\dots, z^{n + 1}) \in \bb{C}^{n + 1}\backslash \{0\} \text{ and } z^i \neq 0\}
    \] 
    Consider the map $\sigma_i: U_i \to M$ satisfying $$\sigma_i[z^1,\dots, z^{n + 1}] = \left(\frac{z^1}{z^i},\dots,\frac{z^{n + 1}}{z^i}\right).$$ In fact, we already proved that $\sigma_i$ is well-defined and a homeomorphism. Moreover, $\sigma_i$ is a local section of $\pi$ since we have 
    \[
        \pi \circ \sigma_i[z^1,\dots, z^{n + 1}] =  \left[\frac{z^1}{z^i},\dots,\frac{z^{n + 1}}{z^i}\right] = [z^1,\dots,z^{n + 1}] = \mathrm{Id}_{\sigma_i(N)}
    \] As the domain of $\{(U_i,\sigma_i)\}$ admits an open cover of $\bb{C}^{n + 1} \backslash\{0\}$, then every point of $\bb{C}^{n + 1} \backslash\{0\}$ is in the image of some smooth local section of $\pi$. Therefore $\pi$ is a surjective smooth submersion.
\end{proof}
\begin{problem}
...
\end{problem}
\begin{proof}
    For the sake of condition, let $\pi_1: M \to N$ be a surjective smooth submersion and $\pi_2: M \to \ti{N}$ be the surjective map that agrees with $\pi_1$ on any points in $M$. Since the map $\mathrm{Id}_N: N \to \ti{N}$ is a bijection, it thus have inverse. Since we have 
    \[
    \mathrm{Id}^{-1}_N \circ \pi_2 = \pi_1
    \]
    which is a smooth map, then the above hypothesis implies that $\mathrm{Id}^{-1}_N$ is smooth. Let $i: \ti{N} \to \ti{N}$ be the inclusion, since $\ti{N}$ is a smooth manifold, then $i$ is smooth. Hence it follows that $i \circ \pi_2 = \pi_2$ is smooth. Analogously, the map
    \[
        \mathrm{Id}_N \circ \pi_1 = \pi_2
    \]
    is smooth.  Apply the theorem 4.29, we obtain that $\mathrm{Id}_N$ is smooth. Therefore, $\mathrm{Id}_N$ is a diffeomorphism.
\end{proof}
\begin{problem}
...
\end{problem}
\begin{proof}
    For every $t \in \bb{R}$, just pick $\pi(t,1) = t$, then $\pi$ is surjective. Since the maps $f,g: \bb{R}^2 \to \bb{R}$ satisfying $f(x,y) = x$ and $g(x,y) = y$ is smooth, then $\pi$ is also smooth. Let $F: \bb{R} \to P$ the any map. The fact $F$ is smooth implies that the composition $F \circ \pi: \bb{R}^2 \to P$ is also smooth. Now we assume that $F \circ \pi$ is smooth, then the resitricted map $F \circ \pi : \bb{R} \times \{1\} \to \bb{R}$ is smooth, hence $F$ is smooth. Let $p = (0,0) \in \bb{R}^2$ and $v_p \in T_p\bb{R}^2$, suppose $d\pi_p(v_p) = w_{\pi(p)} \neq 0$, we have 
    \[
        D\pi_p(v_p) = \begin{bmatrix}
            y & x
        \end{bmatrix}\begin{bmatrix}
            v^1 \\ v^2
        \end{bmatrix} = w_{\pi(p)}
    \]
    Then it follows that $w_{\pi(p)} = 0$, thus $\pi$ is not a smooth submersion.
\end{proof}
...
\end{problem}
\begin{problem}
    Show that the map $q: \mathbb{S}^n \rightarrow \mathbb{R} \mathbb{P}^n$ defined by
    \[
        q(x^1,\dots,x^n) = [x^1,\dots,x^n]
    \] is a smooth covering map. 
\end{problem}
\begin{proof}
    We already proved that $q$ is smooth and surjective. For each $x \in \bb{S}^n$, consider the neighborhood $U_x$ containing $x$ where $U_x$ is abitrarily small such that $U_x \cap (-U_x) = \varnothing$ The fact that $q(a) = q(b)$ implies that $a = \pm b$, since $U_x \cap (-U_x) = \varnothing$. Then it follows that  $a = b$ and $q$ is injective. Then the restricted map $q: U_x \to q(U_x)$ is diffeomorphism since it is also smooth and has smooth inverse. Let $V_x = q(U_x)$, since we have 
    \[
        q^{-1}(V_x) = U_x \sqcup U_{-x}
    \]
    and each of component of $q^{-1}(V_x)$ is mapped diffeomorpically into corresponding $U_x$ and $U_{-x}$, hence $q$ is a smooth covering map.
\end{proof}
\begin{problem}
...
\end{problem}
\begin{proof}
    Let $U = \{[x,y,z] \mid (x,y,z) \in \bb{S}^2\}$, then we have $U = \bb{RP}^4$. Consider the map $\ti{F}: U \to \bb{R}^4$ be the map satisfying $F = \ti{F} \circ \pi$. Then we have 
    \[
    \ti{F}[x,y,z] = (x^2 - y^2,xy,xz,yz),
    \]
    Where $x^2 + y^2 + z^2 = 1$. Since $[x,y,z] =[\lambda x, \lambda y, \lambda z]$ implies that $\lambda = \pm 1$, then it follows that 
    \[
        \ti{F}[-x,-y,-z] = (x^2 - y^2,xy,xz,yz) = \ti{F}[x,y,z].
    \]
    Thus $\ti{F}$ is well-defined. Since $F$ is smooth and $\pi$ is a smooth covering, then the above proposition follows that $\ti{F}$ is smooth. It is clear to show that $\ti{F}$ is bijective onto $\ti{F}(\bb{R}^4)$. Let $p = (x,y,z)$ and $v_p = v^i\dfrac{\pa}{x^i}\bigg|_p \in T_p(\bb{S}^2)$ be abitrary.  Suppose we have $d\ti{F}_{[p]}([v_p]) = 0$, it follows that 
    \[
        dF_p(v_p) = d\ti{F}_{[p]}\circ d\pi_p(v_p) = d\ti{F}_{[p]}([v_p]) = 0
    \]
    Computing the Jacobian $DF_p$ we obtain
    \[
        DF_p(v_p) =\begin{bmatrix}
            2x & -2y & 0\\
            y & x & 0\\
            z& 0 & x\\
            0 & z & y
        ...
        \end{bmatrix}
    \]
    Since $x,y,z$ cannot vanish simultaneously, then the only solution for $dF_p(v) = 0$ is $v = 0$, thus $\kernel(d\ti{F}_p) =\{0\}$. Therefore, $\ti{F}$ is an immersion and since $\bb{RP}^2$ is compact, $\ti{F}$ must be a smooth embedding.
\end{proof}