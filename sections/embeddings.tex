...
\begin{proof}
    ...
    \end{align*}
    Then one can view $F(x,y) = (F_1(x,y),F_2,(x,y))$ for some smooth map $F_1: U \to \bb{R}^r$ and $F_2: V \to \bb{R}^{n - r}$. Let $p=(x_p,y_p)$ and $\varphi: U \to \bb{R}^m$ satisfying $\varphi(x,y) = (F_1(x,y),y)$, we have 
    \[
        ...
        \end{bmatrix}
    \]
    The above assumption implies that $DF_1(x,y)$ and $\delta_i^j$ are invertable, then $D\varphi(x_p,y_p)$ is invertable.  Apply the inverse function theorem for the map $\varphi$ and $p \in U$, there exists connected neighborhoods $U_p$ and $V_{F_1(p)}$ such that $\varphi|_{U_p}: U_p \to \varphi(U_p)$ is a diffeomorphism. Then we can consider the local chart $(U_p,\varphi)$ which is a restricted diffeomorphism. On the following restriction, we can let $A,B: \varphi(U_p)\to U_p$ be the smooth function satisfying $\varphi^{-1}(x,y) = (A(x,y),B(x,y))$, then we obtain 
    \[
    ...
    \]
    where $C: \varphi(U_p) \to \bb{R}^{n - r}$ defined by $C(x,y) = F_2(A(x,y),y)$
\end{proof}
