...
\begin{proof}
    ...
    Since $S$ is connected and compact, then the Classification theorem establishes that $S$ is homeomorphic to $\bb{S}^1$, denoted by the map $F: S \to \bb{S}^1$. Let $p \in \bb{S}^1$, since $ \bb{S}^1 \backslash \{p\}$ is still a connect manifold, whereas $S\backslash \{F(p)\}$ is not, then $S$ cannot be embbeded into a submanifold of $\bb{R}^2$.
\end{proof}
...
\end{problem}
\begin{proof}
    Solving the equation $DF(x,y) = 0$ yields $(x,y)$ equals to $(0,0)$ or $\left(\frac{-1}{3},\frac{-1}{3}\right)$. We have $F(0,0) = 0$ and $F(\frac{-1}{3},\frac{-1}{3}) =\frac{-1}{27}$, then for any $c \neq 0,\frac{-1}{27}$, the level set $F^{-1}(c)$ is regular, thus it is embbeded submanifold of $\bb{R}^2$. We now consider two following cases:

    If $c = 0$, suppose $F^{-1}(0)$ is an embbeded submanifold. Consider the equation 
    \begin{equation}
           x^3 + xy + y^3 = 0
    \end{equation}
    where $y$ is solution by $x$. Since for $x$ large enough, it only has 1 root $y$, then it must be $1$-dimensional embbeded submanifold of $\bb{R}^2$. Since $F^{-1}(0)$  is closed but unbounded, by the Heine-Borel theorem, it is not compact. Then the classification theorem follows that $F^{-1}(0)$ is homeomorphic to $\bb{R}$. Define the following polar coordinate $(x,y) \mapsto (r\cos(\phi), r\sin(\phi))$, then it follows that 
    \[
        F(x,y) = x^3 + xy + y^3 = r^2( \cos(\phi)^3 + r\sin(\phi)\cos(\phi) + \sin(\phi)^3)
    \]
    Let $I = [0,2\pi)\backslash\{\frac{-\pi}{4},\frac{3\pi}{4}\}$, then we can define $r: I \to \bb{R}$ as the embbeded curve on $F^{-1}(0)$ satisfying 
    \[
        r(\phi) = \frac{-\sin(\phi)\cos(\phi)}{\sin(\phi)^3 + \cos(\phi)^3}, \phi \in I.
    \]
    Since $(0,0)$ is the only critical point of $F^{-1}(0)$, we aim to estimate how many times $r$ passes through this point as a curve. Let $r(\phi) = 0$, then we have 
    \[
        -\sin(\phi)\cos(\phi) = \frac{-\sin(2\phi)}{2} = 0 \lra \phi = \frac{k\pi}{2}, k \in \bb{Z}
    \]
    Set $\phi \in I$, then the following solution is $0,\dfrac{\pi}{2},\pi, \dfrac{3\pi}{2}$. Moreover, we also have 
    \[
        \limit_{\phi \to 0}r(\phi) = \limit_{\phi \to \frac{\pi}{2}}r(\phi) =\limit_{\phi \to \pi}r(\phi) = \limit_{\phi \to \frac{3\pi}{2}}r(\phi) = 0
    \]
    Now we divide the domain of $r$ into 4 sections, which is 
    \[
        I_1 = \left[0,\frac{\pi}{2}\right), I_2 = \left[\frac{\pi}{2},\pi\right],I_3 = \left[\pi,\frac{3\pi}{2}\right), I_4 = \left[\frac{3\pi}{2},2\pi\right)
    \]
    For each $i = 1,2,3,4$, define the restriction $r_i: I_i \to \bb{R}_{\geq 0}$ that agrees with $r$ on $I_i$. Then we have $r_1(\phi + \pi) = -r_3(\phi)$
\end{proof}
\begin{problem}
